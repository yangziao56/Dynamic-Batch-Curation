...
parser.add_argument('--method', type=str, default='Ghost', help='vanilla, SPL, IP, Ghost, output_grads_dot, self_output_grads_dot')
# SPL parameters
...
# for _ in range(5):
    ...
    for epoch in range(epochs):
        ...
        for data, target, idx in train_loader:
            ...
            if args.method == 'vanilla':
            ...
            elif args.method == 'SPL' and epoch > 9:
                output = model(data)
                losses = criterion(output, target)  # Compute individual losses for SPL
                ...
                '''
            elif args.method == 'Ghost' and epoch > 1:
                from fvcore.nn import FlopCountAnalysis

                # 1) 自动化统计 - 仅对 forward pass 做示例
                data_stack = torch.cat((data, data), dim=0)  # 构造一个 dummy data_stack
                dummy_input = data_stack.clone().detach()
                auto_flops_forward = FlopCountAnalysis(model, dummy_input).total()
                print(f"[Ghost] Auto FLOPs (forward pass) for one batch: {auto_flops_forward}")

                # 手动近似统计：forward FLOPs + backward FLOPs(约2倍forward)
                manual_flops_forward = auto_flops_forward  
                manual_flops_backward = 2 * auto_flops_forward  
                ...
                def forward_hook(module, input_, output_):
                    output_.retain_grad()  # 保留梯度
                    layer_outputs[module] = (input_[0], output_)
                ...
                # 这里只演示一个 batch
                data2, target2, _ = next(iter(val_subset_loader))
                data2, target2 = data2.to(args.device), target2.to(args.device)
                ...
                N_train, D = output_train_grads.shape
                N_val = output_val_grads.shape[0]

                # 2) 手动统计 grads_dot FLOPs（整个输出）
                manual_flops_gradsdot = 2 * N_train * D * N_val
                print(f"[Ghost] Manual FLOPs for grads_dot (one batch): {manual_flops_gradsdot}")

                # -----------------------------------------------------------------------
                # 3) 多层输入点积的 FLOPs，需要累加
                total_inputdot_flops = 0  # 用于多层累加
                for layer, (layer_input, layer_output) in layer_outputs.items():
                    if layer_output.grad is None:
                        continue
                    
                    # 训练部分梯度、验证部分梯度
                    output_train_grads_ = layer_output.grad[:data.shape[0]]
                    output_val_grads_ = layer_output.grad[data.shape[0]:]
                    ...
                    # 大小信息
                    M, K = input_train.shape
                    N_ = input_val.shape[0]

                    # 点积的 FLOPs
                    manual_flops_inputdot = 2 * M * K * N_
                    total_inputdot_flops += manual_flops_inputdot  # 累加到 total_inputdot_flops

                    # 计算 ghost
                    grads_dot_ = torch.mm(output_train_grads_.view(M, -1),
                                        output_val_grads_.view(N_, -1).T)
                    input_dot = torch.mm(input_train, input_val.T)
                    ghost_total += (grads_dot_ * input_dot).mean(dim=1)
                ...
                # 4) 汇总手动FLOPs：
                #    (前向 + 反向) + grads_dot (输出) + input_dot (所有层累加)
                manual_flops = manual_flops_forward + manual_flops_backward + manual_flops_gradsdot + total_inputdot_flops
                print(f"[Ghost] Approx. manual total FLOPs (one batch) = {manual_flops}\n")

            elif args.method == 'Ours' and epoch > 1:
                # 同理，在这里插入自动/手动 FLOPs 统计（仅示例单batch）
                from fvcore.nn import FlopCountAnalysis
                ...
                dummy_input = data_stack.clone().detach()
                auto_flops_forward = FlopCountAnalysis(model, dummy_input).total()
                print(f"[Ours] Auto FLOPs (forward pass) for one batch: {auto_flops_forward}")

                # --------------前向&反向--------------
                output = model(data_stack)
                output.retain_grad()
                losses_total = criterion(output, target_stack)
                # 手动近似：forward + backward
                manual_flops_forward = auto_flops_forward
                manual_flops_backward = 2 * auto_flops_forward

                output_grads = torch.autograd.grad(outputs=losses_total.mean(), 
                                                   inputs=output, 
                                                   retain_graph=True)[0]
                ...
                N_train, D = output_train_grads.shape
                N_val = output_val_grads.shape[0]
                flops_gradsdot = 2 * N_train * D * N_val
                print(f"[Ours] Manual FLOPs for grads_dot (one batch): {flops_gradsdot}")
                ...
                for layer, (layer_input, layer_output) in layer_outputs.items():
                    ...
                    M, K = input_train.shape
                    N_ = input_val.shape[0]
                    flops_inputdot = 2 * M * K * N_
                    print(f"[Ours] Manual FLOPs for input_dot in one layer: {flops_inputdot}")
                    input_dot_sum += torch.mm(input_train, input_val.T)

                for hook in hooks:
                    hook.remove()

                grads_dot = torch.mm(output_train_grads.view(N_train, -1),
                                     output_val_grads.view(N_val, -1).T)
                ...
                # --------------手动FLOPs合计近似--------------
                manual_flops = manual_flops_forward + manual_flops_backward + flops_gradsdot
                print(f"[Ours] Approx. manual total FLOPs (one batch) = {manual_flops}\n")
            ...
            elif args.method == 'Ours_last_layer' and epoch > 1:
                from fvcore.nn import FlopCountAnalysis
                ...
                dummy_input = data_stack.clone().detach()
                auto_flops_forward = FlopCountAnalysis(model, dummy_input).total()
                print(f"[Ours_last_layer] Auto FLOPs (forward pass) for one batch: {auto_flops_forward}")
                ...
                ghost_total = 0  
                layer_outputs = {}
                def last_layer_hook(module, input_, output_):
                    layer_outputs[module] = (input_[0], output_)
                
                hooks = []
                for name, layer in model.named_modules():
                    if name == 'fc':  # 替换成你模型最后一层名称
                ...
                for layer, (layer_input, layer_output) in layer_outputs.items():
                    ...
                    # 手动统计： grads_dot + input_dot
                    flops_gradsdot = 2 * N_train * D * N_val
                    flops_inputdot = 2 * input_train.shape[0] * input_train.shape[1] * input_val.shape[0]
                    print(f"[Ours_last_layer] Manual FLOPs for grads_dot: {flops_gradsdot}")
                    print(f"[Ours_last_layer] Manual FLOPs for input_dot: {flops_inputdot}")
                ...
                manual_flops = manual_flops_forward + manual_flops_backward + flops_gradsdot + flops_inputdot
                print(f"[Ours_last_layer] Approx. manual total FLOPs (one batch) = {manual_flops}\n")
        ...
        if epoch == 9:
            lambda_ = total_loss * 1.5  / len(train_loader)
