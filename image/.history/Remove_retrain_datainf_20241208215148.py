...
for batch_idx, (images, labels, _) in enumerate(tqdm(temp_dl_train, desc="Training Progress")):
    ...
    # print("accumulated_grads_val.shape", average_grads_val.shape)
    # print(grads.shape)
    temp_grads.append(grads.unsqueeze(0).cpu())  # 先存入临时列表
    #grads_list.append(grads.unsqueeze(0).cpu())  # Shape: (1, total_grads)
    if (batch_idx + 1) % 100 == 0:
        grads_list.append(torch.cat(temp_grads))  # 将 N 个 Batch 的梯度拼接
        temp_grads = []  # 清空临时梯度列表

    if (batch_idx + 1) % 1000 == 0:
        grads_list = torch.cat(grads_list)  # Shape: (N, total_grads)

# 处理剩余的梯度
if len(temp_grads) > 0:
    grads_list.append(torch.cat(temp_grads))
...
grads_val = accumulated_grads_val.cpu().numpy()
