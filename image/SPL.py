...
parser.add_argument('--subset_ratio', type=float, default=0.1, help='')
...
# for _ in range(5):
    ...
    score_matrix_input_dot_sum = np.zeros((50000, int(5000 * args.subset_ratio)))
    score_matrix_grads_dot = np.zeros((50000, int(5000 * args.subset_ratio)))
    ...
    for epoch in range(epochs):
        ...
        for data, target, idx in train_loader:
            ...
            if args.method == 'vanilla':
                ...
                # 计算10%的样本数
                subset_size = int(args.subset_ratio * len(all_data))
                ...
                for data2, target2, _ in val_subset_loader:
                    ...
                    # # save score_matrix with dataset, noise_type, method, epoch
                    # np.save(f'{args.dataset}_{args.noise_type}_{args.method}_score_matrix.npy',score_matrix)
        ...
        max_allocated = torch.cuda.max_memory_allocated(args.device) / (1024**2)
