...
accumulated_grads_val = torch.zeros(total_params).to(args.device)  # 假设total_params是正确的
...
# params = {k: v.detach() for k, v in inf_model.named_parameters()}
# buffers = {k: v.detach() for k, v in inf_model.named_buffers()}
...
#     logits = functional_call(inf_model, (params, buffers), (image,))
...
for images, labels, _ in tqdm(temp_dl_train):
    ...
    # 计算内积
    inner_product = torch.matmul(grads, average_grads_val)
    #print(inner_product.shape)
    IP.append(inner_product.unsqueeze(0))
IP = torch.cat(IP)
print("IP shape", IP.shape)
...
with open('result.txt', 'a') as f:
    ...
    f.write(f'Method: IP, dataset: {args.dataset}, noise: {args.noise_type}, Average test accuracy: {avg_test_acc * 100:.2f}%, std: {std_test_acc * 100:.2f}%\n')
