...
parser.add_argument('--seed_previous', type=int, default=41, help='Random seed')
...
temp_dl_train = DataLoader(train_dataset, batch_size=32, shuffle=True)
#train_loader = DataLoader(clean_train_subset, batch_size=args.batch_size, shuffle=True)
temp_dl_val = DataLoader(val_dataset, batch_size=32, shuffle=False)
train_labels = [train_dataset.dataset.train_labels[i] for i in train_dataset.indices]
val_labels = [val_dataset.dataset.train_labels[i] for i in val_dataset.indices]
#load the ckpts
...
if args.dataset == 'cifar100':
    ...
    if args.noise_type == 'aggre_label':
        checkpoint_paths = [
            "ckpt/cifar10_vanilla_aggre_epoch9.pt",
            "ckpt/cifar10_vanilla_aggre_epoch29.pt",
            "ckpt/cifar10_vanilla_aggre_epoch69.pt",
            "ckpt/cifar10_vanilla_aggre_epoch149.pt",
            "ckpt/cifar10_vanilla_aggre_epoch309.pt"
        ]
        extra_seed = 110
    elif args.noise_type == 'worse_label':
        checkpoint_paths = [
            "ckpt/cifar10_vanilla_worst_epoch9.pt",
            "ckpt/cifar10_vanilla_worst_epoch29.pt",
            "ckpt/cifar10_vanilla_worst_epoch69.pt",
            "ckpt/cifar10_vanilla_worst_epoch149.pt",
            "ckpt/cifar10_vanilla_worst_epoch309.pt"
        ]
        extra_seed = 120
    else:
        checkpoint_paths = [
            "ckpt/cifar10_vanilla_rand1_epoch9.pt",
            "ckpt/cifar10_vanilla_rand1_epoch29.pt",
            "ckpt/cifar10_vanilla_rand1_epoch69.pt",
            "ckpt/cifar10_vanilla_rand1_epoch149.pt",
            "ckpt/cifar10_vanilla_rand1_epoch309.pt"
        ]
        extra_seed = 130

if args.dataset == 'cifar100':
    checkpoint_lr = [0.01, 0.01, 0.01, 0.01]
    model_ids = [extra_seed, extra_seed+1, extra_seed+2, extra_seed+3]
else:
    checkpoint_lr = [0.01, 0.01, 0.01, 0.01, 0.01]
...
filename_memmap = f"result_Trak/Trak-{args.dataset}-{args.seed}.memmap"
...
threshold = 0
del_idxs = []
for i, ip in enumerate(Trak_scores):
    if ip <= threshold:
        del_idxs.append(i)
...
with open('result.txt', 'a') as f:
    ...
    f.write(f'Method: Trak, dataset: {args.dataset}, noise: {args.noise_type}, Average test accuracy: {avg_test_acc * 100:.2f}%, std: {std_test_acc * 100:.2f}%\n')
