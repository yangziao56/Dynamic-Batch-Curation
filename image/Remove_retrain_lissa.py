...
threshold = 0
del_idxs = []
for i, ip in enumerate(inf_eng.IF_dict['LiSSA']):
    if ip <= threshold:
        del_idxs.append(i)
...
def sample_remove_dataloader(ds, idx_list, bs=128):
    ...
    dl = DataLoader(dataset = train_dataset,
                                ...
                                drop_last = True, # drop last incomplete batch, used to be False
                                sampler=sampler)
    return dl
...
with open('result.txt', 'a') as f:
    ...
    f.write(f'Method: Lissa, dataset: {args.dataset}, noise: {args.noise_type}, Average test accuracy: {avg_test_acc * 100:.2f}%, std: {std_test_acc * 100:.2f}%\n')
